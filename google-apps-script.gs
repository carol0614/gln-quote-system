...
function setupSummarySheet() {
  ...
  // ① 基礎資料 QUERY（A1，拉 12 欄顯示）
  const queryFormula = '=QUERY({\'線索\'!A:AS}, "SELECT Col1, Col3, Col4, Col45, Col5, Col43, Col6, Col7, Col11, Col15, Col18, Col39 WHERE Col1 IS NOT NULL ORDER BY Col1 DESC LABEL Col1 \'時間\', Col3 \'姓名\', Col4 \'電話\', Col45 \'LINE ID\', Col5 \'案型\', Col43 \'建案/地址\', Col6 \'屋齡\', Col7 \'坪數\', Col11 \'系統預估總價(萬)\', Col15 \'期待預算(萬)\', Col18 \'入住月\', Col39 \'備註\'", 1)';
  sheet.getRange('A1').setFormula(queryFormula);

  // ② 紅旗計數（M1）— 從線索分頁背景撈估價下限、屋況加成
  const flagFormula = '={"🚩 紅旗"; ARRAYFORMULA(IF(A2:A="", "", IF(REGEXMATCH(IFERROR(VLOOKUP(A2:A, \'線索\'!A:S, 19, FALSE), "")&"", "full_demolish|structural"), 1, 0) + IF((J2:J>0)*(J2:J<IFERROR(VLOOKUP(A2:A, \'線索\'!A:I, 9, FALSE), 99999)*0.8), 1, 0) + IF((K2:K<>"")*(IFERROR(DATEVALUE(K2:K&"-01"), 0)-TODAY()<180)*(IFERROR(DATEVALUE(K2:K&"-01"), 0)-TODAY()>0), 1, 0)))}';
  sheet.getRange('M1').setFormula(flagFormula);

  // ③ 分級（N1）— 用 I 系統預估總價、M 紅旗數
  const gradeFormula = '={"🎯 分級"; ARRAYFORMULA(IF(A2:A="", "", IF(I2:I>=1000, "🚨 必轉 Carol", IF(((E2:E="new_house")*(I2:I<100))+(((E2:E<>"new_house")*(E2:E<>""))*(I2:I<150))>=1, "D 轉 GLV", IF(I2:I<400, "C 訓練組", IF(M2:M>=2, "B 必轉設計總監", IF(I2:I>=700, "A+ 頂級", "A 主力")))))))}';
  sheet.getRange('N1').setFormula(gradeFormula);

  // ④ 建議行動（O1）
  const actionFormula = '={"🚦 建議行動"; ARRAYFORMULA(IF(A2:A="", "", IF(N2:N="🚨 必轉 Carol", "Carol／Hank 24h 內親自聯絡", IF(N2:N="B 必轉設計總監", "設計總監 + Carol 預先過案", IF(N2:N="A+ 頂級", "主力設計師 24h 內接洽 + 預約丈量", IF(N2:N="A 主力", "主力設計師 48h 內接洽", IF(N2:N="C 訓練組", "訓練組設計師 72h 內接洽", IF(N2:N="D 轉 GLV", "婉拒並推 GLV 系統櫃方案", "—"))))))))}';
  sheet.getRange('O1').setFormula(actionFormula);

  // 標題列樣式
  sheet.setFrozenRows(1);
  sheet.getRange('A1:O1').setFontWeight('bold').setBackground('#7C837B').setFontColor('#FFFFFF');

  // 條件式格式（依分級整列染色，依 N 分級欄）
  const formatRange = sheet.getRange('A2:O1000');
  const rules = [
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="🚨 必轉 Carol"').setBackground('#FFCDD2').setRanges([formatRange]).build(),
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="B 必轉設計總監"').setBackground('#FFE0B2').setRanges([formatRange]).build(),
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="A+ 頂級"').setBackground('#C8E6C9').setRanges([formatRange]).build(),
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="A 主力"').setBackground('#E8F5E9').setRanges([formatRange]).build(),
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="C 訓練組"').setBackground('#E3F2FD').setRanges([formatRange]).build(),
    SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=$N2="D 轉 GLV"').setBackground('#F5F5F5').setRanges([formatRange]).build()
  ];
